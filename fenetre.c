...
{
case 0:
printf("%d\n",p->perso.positioninscrenn.x);
if(p->perso.positioninscrenn.x<580)
 p->perso.positioninscrenn.x+=0.2*p->acceleration*(p->dt/2)*(p->dt) +0.5*p->vitesse*p->dt; 
    break;
case 1:
if(p->perso.positioninscrenn.x>0)
p->perso.positioninscrenn.x-=0.2*p->acceleration*(p->dt/4)*(p->dt)  ; 
